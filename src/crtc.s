...
@backspace_not_pressed:
    PHA ; preserve caller's state
    PHX
    PHY
    LDY #$00 ; set 0 to scratch data, 1 indicates clear current line
    ; check if it's the enter key
    cmp #$0D
    bne @enter_key_not_pressed
; here enter key was pressed...
    LDY #$01 ; indicate to clear newline and check for scroll
    ...
    PLA
    clc
    adc VIDEO_CHAR_PTR ; add value in low byte memory ptr to the amount we need to add
    bcc @no_low_255_rollover
    inc VIDEO_CHAR_PTR+1 ; we rolled over 255 on low byte
@no_low_255_rollover:
    sta VIDEO_CHAR_PTR
    lda #$00
    sta CURSOR_X_POS
...
@enter_key_not_pressed:
    sta (VIDEO_CHAR_PTR)
    inc VIDEO_CHAR_PTR  ;increment cursor video memory position
    LDA #$01
    JSR MOVE_CURSOR_POSITION
    ...
    LDY #$01 ; indicate to clear current line
    stx CURSOR_X_POS
    lda CURSOR_Y_POS
    cmp #MAX_LINE
    beq @check_256_vid_rollover ; Don't inc y cursor, already at bottom of screen
    inc CURSOR_Y_POS
...
@exit_video_write_char:
    cpy #$00 ; do we need to clear line and check scroll?
